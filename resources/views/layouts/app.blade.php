...
<head>
    ...
    <style>
        ...
        .top-navbar .user-btn:hover { background:rgba(255,255,255,.18); }
        ...
        .top-navbar .dropdown-item:hover { background:rgba(255,255,255,.1); color:#fff; }
        .top-navbar .dropdown-item.text-danger { color:#f87171 !important; }
        .top-navbar .dropdown-item.text-danger:hover { background:rgba(239,68,68,.15); }
        .top-navbar .dropdown-divider { border-color:rgba(255,255,255,.1); margin:4px 0; }
...
{{-- Desktop Navbar --}}
<nav class="navbar navbar-dark bg-dark px-3 py-2 top-navbar">
    <a class="navbar-brand fw-bold fs-6 me-4" href="{{ route('dashboard') }}">
        🏠 Room Rental
    </a>
    <div class="navbar-nav flex-row align-items-center gap-1 me-auto">
    ...
    <div class="dropdown ms-3">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            <li>
                <span class="dropdown-item disabled" style="font-size:11px;opacity:.5">
                    {{ auth()->user()->email ?? '' }}
                </span>
            </li>
...
<nav class="bottom-nav">
    ...
    <a href="{{ route('tenants.index') }}"
       class="{{ request()->routeIs('tenants.*') ? 'active' : '' }}">
    ...
    <a onclick="document.getElementById('mobileLogout').submit()">
        <i class="bi bi-box-arrow-right"></i>Logout
    </a>
</nav>

<form id="mobileLogout" method="POST" action="{{ route('logout') }}" style="display:none">
    @csrf
</form>
